...
class CascadeSessionState:
    ...
    # Lifetime count — kept for backward-compat summary output.
    flagged_turns:    int = 0
    flagged_windows:  int = 0
    total_turns:      int = 0
    stage2_calls:     int = 0
    # BUG-6 FIX: per-turn flag log enables sliding-window persistence check.
    flagged_turn_log: List[Dict[str, Any]] = field(default_factory=list)
...
class CascadeBouncer:
    ...
    def __init__(
        ...
        # ── Persistence knobs (BUG-6 FIX) ───────────────────────────────────
        persistence_min_hits:  int   = 2,
        persistence_window:    int   = 6,    # default = persistence_min_hits * 3
    ...
    def _check_escalation(
        ...
        peak_size = min(len(scores), self.escalation_window)
        window    = scores[-peak_size:]
        ...
        if len(scores) >= self.escalation_window:
            full_window = scores[-self.escalation_window:]
